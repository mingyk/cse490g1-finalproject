...
from src import clf_base
from src import bilstm
from src.constants import DEV_FILE, OFFSET, TRAIN_FILE, UNK
import operator
import os
...
#         raise NotImplementedError
        output = []
        for word in words:
            output.append(clf_base.predict({word:1},weights,all_tags)[0])
        return output
...
def eval_tagger(tagger,outfilename,all_tags=None,trainfile=TRAIN_FILE,testfile=DEV_FILE):
    ...
    score = scorer.get_confusion(testfile,outfilename) #run the scorer on the prediction file
