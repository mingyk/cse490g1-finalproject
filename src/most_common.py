...
from src.constants import OFFSET, START_TAG, END_TAG, UNK
...
#     raise NotImplementedError
    ...
    for tag in all_counters:
        for word in all_counters[tag]:
            weights[(tag, word)] = all_counters[tag][word]
        offsets[(tag, OFFSET)] = len(list(all_counters[tag].elements()))
    most_common = max(offsets, key=lambda key: offsets[key])
    weights[most_common] = 0.1
    return weights
    ...
    for words, tags in conll_seq_generator(trainfile):
        for i, (word, tag) in enumerate(zip(words, tags)):
            ...
            if i == len(tags) - 1:
                ...
                tot_counts[tag].update([tags[i+1]])
